...
public class PricingController {
    ...
    @PostConstruct
    public void init() {
        log.info("PricingController initialized with MarketDataProvider: {}", 
                 marketDataProvider.getClass().getName());
        log.info("Is ResilientMarketDataProvider? {}", 
                 marketDataProvider instanceof com.srviswan.basketpricing.resilience.ResilientMarketDataProvider);
    }

    @GetMapping
    @RateLimiter(name = "pricingApi")
    ...
    public ResponseEntity<Map<String, PriceSnapshot>> getPrices(@RequestParam("symbols") String symbolsCsv) {
        ...
        try {
            ...
            if (backpressureManager.getQueueUtilization() > 0.8) {
                log.warn("High backpressure detected: {}% queue utilization", 
                    backpressureManager.getQueueUtilization() * 100);
            }
    ...
    public ResponseEntity<Map<String, Object>> subscribe(@RequestParam("symbols") String symbolsCsv) {
        ...
        try {
            ...
            response.put("backpressureStatus", Map.of(
                "queueUtilization", backpressureManager.getQueueUtilization(),
                "processedUpdates", backpressureManager.getProcessedUpdates(),
                "droppedUpdates", backpressureManager.getDroppedUpdates()
            ));
